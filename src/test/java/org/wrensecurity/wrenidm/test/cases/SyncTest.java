...
public class SyncTest extends BaseWrenidmTest {
    ...
    public void testImplicitSync() throws Exception {
        ...
        while (true) {
                ...
                HttpRequest req = HttpRequest.newBuilder(URI.create(WRENIDM_BASE_URL + "/openidm/audit/sync?_queryFilter=" + encoded))
                    .header("Authorization", ADMIN_AUTHORIZATION_HEADER_VALUE)
                    .build();
            HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
            assertEquals(200, resp.statusCode());
            ...
            JsonNode body = mapper.readTree(resp.body());
            if (body.get("resultCount").asInt() == 2) {
                break;
            }
            Thread.sleep(1000);
        }
